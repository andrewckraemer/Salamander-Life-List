...
lifelist<-subset(lifelist,lifelist$year_Juniper>2016)
...
last_lo<-tail(lifelist$location_Juniper,n=1)
last_yr<-tail(lifelist$year_Juniper,n=1)
...
label_data$genusnamecolors<-c('palegreen1','cadetblue1','cadetblue1','cadetblue1','cadetblue1','palegreen1','cadetblue1','cadetblue1','palegreen1','cadetblue1','cadetblue1','cadetblue1','cadetblue1','cadetblue1','cadetblue1','palegreen1','cadetblue1','cadetblue1','cadetblue1','cadetblue1','cadetblue1','cadetblue1','cadetblue1')
...
p <- ggplot(data) +      
  ...
  scale_fill_manual(values=c('cadetblue1','lightpink')) +
  ...
  geom_text(data=label_data, aes(x=id.x, y=-12, label=paste(seen,'/',tot,sep=''), hjust= hjust), color= 'palegreen1', fontface="bold",alpha=1, size=3, angle= label_data$angle, inherit.aes = FALSE )

# accumulation curve
accumulation<-c(0,cumsum(table(lifelist$year_Juniper)),max(cumsum(table(lifelist$year_Juniper))))
newyears<-as.numeric(c(as.numeric(min(names(table(lifelist$year_Juniper))))-1,names(table(lifelist$year_Juniper)),format(Sys.Date(), "%Y")))
...
finalgraph<- graph + draw_text('Yet to See', x=0.8,y=0.94,size=22,color='cadetblue1',fontface='bold',hjust=0)+ draw_text('Seen', x=0.8,y=0.90,size=22,color='lightpink',fontface='bold',hjust=0)+ draw_text(paste(spseen,'of',sptotal), x=0.23,y=0.84,size=18,color='black',fontface='bold',hjust=0)+ 
...
ggsave(finalgraph, file="SalamanderLifeListJuniper.png", width=10, height=10)
